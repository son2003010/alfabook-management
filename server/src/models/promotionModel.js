...
class PromotionModel {
    ...
    static async isPromotionIdExists(promotionId) {
        ...
        const result = await request.query('SELECT COUNT(*) AS count FROM Promotion WHERE PromotionID = @promotionId');
        return result.recordset[0].count > 0;
    }

    static async addPromotion(promotionName, description, startDate, endDate, discount, isActive = true) {
        const request = new sql.Request();
        ...
        const result = await request
            .query(`INSERT INTO Promotion (PromotionName, Description, StartDate, EndDate, Discount, IsActive, CreatedDate, UpdatedDate) 
    ...
    static async updatePromotion(promotionId, promotionName, description, startDate, endDate, discount, isActive) {
        const request = new sql.Request();
        request.input('promotionId', sql.Int, promotionId);
        request.input('promotionName', sql.NVarChar, promotionName);
        request.input('description', sql.NVarChar, description);
