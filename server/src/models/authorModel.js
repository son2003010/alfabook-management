...
class AuthorModel {
  ...
  static async isAuthorIdExists(authorId) {
    ...
    const result = await request.query(
      'SELECT COUNT(*) AS count FROM Author WHERE AuthorID = @authorId',
    );
  ...
  static async addAuthor(authorId, authorName, description) {
    ...
    const result = await request.query(
      'INSERT INTO Author (AuthorID, AuthorName, Description) OUTPUT INSERTED.AuthorID VALUES (@authorId, @authorName, @description)',
    );
  ...
  static async updateAuthor(authorId, authorName, description) {
    ...
    const result = await request.query(
      'UPDATE Author SET AuthorName = @authorName, Description = @description WHERE AuthorID = @authorId',
    );
    return result.recordset;
  }
