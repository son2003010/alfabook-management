...
const AdminAuthor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAuthors((prev) =>
        editingAuthor
          ? prev.map((a) => (a.AuthorID === editingAuthor.AuthorID ? { ...a, ...formData } : a))
          : [...prev, { ...formData }]
      );
    ...
    <div className="p-6">
      <div className="bg-white rounded-lg shadow">
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <button onClick={() => { setIsOpen(true); setEditingAuthor(null); setErrorMessage(''); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
              ...
              {authors.map((author) => (
                <tr key={author.AuthorID} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 flex gap-2">
                    <button onClick={() => { setEditingAuthor(author); setFormData(author); setIsOpen(true); }} className="p-1 text-blue-500 hover:bg-blue-50 rounded">
                      <Edit className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(author.AuthorID)} className="p-1 text-red-500 hover:bg-red-50 rounded">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
            <div className="flex justify-between items-center border-b pb-3">
              <h3 className="text-lg font-semibold">{editingAuthor ? 'Chỉnh sửa Tác Giả' : 'Thêm Tác Giả'}</h3>
