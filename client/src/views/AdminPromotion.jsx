  ...
  const handleSubmit = async (e) => {
    ...
    if (response.ok) {
      ...
      if (editingPromotion) {
        setPromotions(
          promotions.map((pro) =>
            pro.PromotionID === editingPromotion.PromotionID
              ? { ...pro, ...formData }
              : pro
          )
        );
        ...
        setPromotions([
          ...promotions,
          { ...formData, PromotionID: data.PromotionID },
        ]);
      }
      setIsOpen(false);
            ...
            <tbody className="divide-y">
              {promotions.map((promotion) => (
                <tr key={promotion.PromotionID} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">{formatDateForDisplay(promotion.StartDate)}</td>
                  <td className="px-4 py-2">{formatDateForDisplay(promotion.EndDate)}</td>
