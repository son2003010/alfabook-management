    ...
    const fetchBooksByCategory = async () => {
      try {
        const bookPromises = categories.map((category) =>
          fetch(`/api/get-book-by-category/${category.CategoryID}?page=1&pageSize=${booksPerPage}`)
            .then((res) => res.json())
            .then((data) => ({
            ...
            <div className="bg-white rounded-lg shadow-sm p-4">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => scrollContainer(`scroll-container-${category.CategoryID}`, 'left')}
                    className="p-2 rounded-full hover:bg-gray-100 transition-colors"
                  >
                    <ChevronLeft className="w-6 h-6 text-gray-600" />
                  </button>
                  <button 
                    onClick={() => scrollContainer(`scroll-container-${category.CategoryID}`, 'right')}
                    className="p-2 rounded-full hover:bg-gray-100 transition-colors"
                  >
