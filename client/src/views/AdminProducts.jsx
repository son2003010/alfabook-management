...
const AdminProducts = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(
          `/api/get-book-by-category/${selectedCategory}?page=${currentPage}&pageSize=${booksPerPage}`
        );
        ...
        const productsWithDetails = await Promise.all(
          data.books.map(async (book) => {
            try {
              ...
              console.error(
                `Lỗi khi lấy chi tiết sách ID ${book.BookID}:`,
                error
              );
              return book;
            }
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/search-book?query=${searchQuery}`);
                ...
                {searchQuery && searchResults.length > 0 ? (
                  searchResults.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {product.publisherName || product.PublisherName || "Không có"}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {product.authorNames || product.AuthorName || "Không có"}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {product.Discount || 0}%
                      </td>
        ...
        <div className="modal-container">
          <div className="modal-content">
            ...
            >
              ...
              <button
                type="submit"
                className="bg-blue-600 text-white p-2 rounded-md px-6 hover:bg-blue-700 transition col-span-2"
              >
                Thêm sản phẩm
              </button>
            </form>
