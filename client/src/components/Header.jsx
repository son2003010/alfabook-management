...
function Header() {
  ...
  const { cartCount, cartItems, fetchCartCount, fetchCartItems, addToCart, updateQuantity, removeFromCart, clearCart } = useCart();
      ...
      <header className="w-full bg-white shadow-md">
        <div className="container mx-auto flex justify-between items-center py-2 px-4 lg:px-6">
          ...
          <div className="flex items-center flex-shrink-0">
            <a href='/'><img src={logo} alt="Logo" className="h-16 md:h-20 w-auto" /></a>
          </div>
            ...
            {showResults && searchResults.length > 0 && (
              <div className="absolute left-0 w-full bg-white border border-gray-300 shadow-lg rounded-md mt-1 z-50 max-h-60 overflow-auto">
                {searchResults.map((book) => (
                  ...
                  >
                    <img src={getBookImage(book.Image)} alt={book.Title} className="w-10 h-10 object-cover rounded-md mr-3" />
                    <div>
                      <p className="text-sm font-medium">{book.Title}</p>
                      <p className="text-xs text-gray-500">{book.Price.toLocaleString()}₫</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center space-x-4 lg:space-x-6">
              ...
              <div className="flex flex-col items-center group cursor-pointer">
                <div className="p-2 rounded-full group-hover:bg-gray-100 transition-colors duration-200">
                  <svg className="w-6 h-6 text-gray-700 group-hover:text-red-500 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                  </svg>
                </div>
                <span className="text-xs text-gray-700 group-hover:text-red-500 transition-colors duration-200">Thông báo</span>
              </div>
          ...
          <div className="absolute top-full right-0 mt-2 w-96 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
            ...
            {cartItems.length === 0 ? (
              <div className="p-6 text-center">
                ...
                <p className="text-gray-600 text-sm">Giỏ hàng của bạn đang trống</p>
              </div>
            ) : (
              <>
                {/* Items List */}
                <div className="max-h-96 overflow-y-auto">
                  {cartItems.map((item) => (
                    <div key={item.Id} className="flex items-start p-4 border-b border-gray-100 hover:bg-gray-50">
                      <img 
                     ...
                     <div className="flex-1 ml-3">
                          ...
                          <p className="text-red-500 font-bold">{(item.Price * (1 - item.Discount / 100)).toLocaleString()}₫</p>
                        </div>
                  ...
                  <button className="w-full mt-3 bg-red-600 text-white py-2 rounded" onClick={clearCart}>Xóa toàn bộ</button>
                </div>
              ...
              <div className="relative group account-dropdown">
                ...
                >
                  <div className="p-2 rounded-full hover:bg-gray-100 transition-colors duration-200">
                    <svg className="w-6 h-6 text-gray-700 hover:text-red-500 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
